...
def _get_main(log=None):
    ...
    if _ocr_main is None:
        try:
            ...
            if log:
                log.info("[抢红包] ddddocr 默认模型初始化成功")
        except Exception as e:  # noqa: BLE001
            if log:
                log.warning("[抢红包] ddddocr 默认模型不可用: %r", e)
            _ocr_main = False
...
def _get_old(log=None):
    ...
    if _ocr_old is None:
        try:
            ...
            if log:
                log.info("[抢红包] ddddocr 旧版模型初始化成功")
