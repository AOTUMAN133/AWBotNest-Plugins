...
#
# 包含三类红包处理 + 陷阱防护：
#   1. 口令红包（图片/文档口令）：OCR 识别口令自动回复；OCR 失败/关闭 → 等待复制
#      他人被确认的口令再发。三层陷阱防护：命令前缀 / 注入字符 / 关键词库。
#   2. 按钮红包（HDSKY 拼手气红包）：点击「抢红包」内联按钮。
#   3. 编号按钮红包（癫影积分红包）：逐个点击未抢的数字按钮。
#
# 状态机（口令红包）在内存中维护：
...
def is_trap_keyword(keyword: str, enabled: bool, custom_keywords: list[str], log=None) -> bool:
    ...
    if kw.startswith("/") or kw.startswith("."):
        if log:
            log.warning("[抢红包陷阱] 口令以命令前缀开头，拒绝: %r", kw)
        return True
    for cmd in _DANGEROUS_COMMANDS:
        if cmd in kw:
            if log:
                log.warning("[抢红包陷阱] 口令含危险命令 %r，拒绝: %r", cmd, kw)
            return True

    # 2. 命令注入（始终拦截）
    if re.search(r"[\n\r;|&]", kw):
        if log:
            log.warning("[抢红包陷阱] 口令含注入字符，拒绝: %r", kw)
        return True
    ...
    for trap in custom_keywords:
        if trap and trap.lower() in kw_lower:
            if log:
                log.warning("[抢红包陷阱] 口令命中陷阱关键词 %r，拒绝: %r", trap, kw)
            return True
    return False
...
class TokenSnatcher:
    ...
    async def handle_new_packet(
        ...
        if is_trap_keyword(caption, trap_enabled, custom_keywords, self._log) and caption:
            ...
            self._log.info("[抢红包] caption 含可疑词，谨慎处理 chat=%s msg=%s", group_id, packet_id)
        ...
        if not keyword:
            ...
            self._log.info("[抢红包] 进入等待复制 chat=%s msg=%s 原因=%s", group_id, packet_id, reason)
            return

        # OCR 识别成功 → 陷阱检测口令本身
        if is_trap_keyword(keyword, trap_enabled, custom_keywords, self._log):
            self._log.warning("[抢红包] OCR口令命中陷阱，拦截: %r", keyword)
            if notify:
        ...
        try:
            ...
            self._log.error("[抢红包] OCR口令发送失败: %r", e)
            return
        ...
        self._log.info("[抢红包] 已发OCR口令 chat=%s msg=%s sent=%s 口令=%r", group_id, packet_id, sent_id, keyword)
        if notify:
            await self._safe_notify(
                f"抢红包-已发OCR口令\n👤 发包人: {sender_name}\n🔑 口令: {keyword}\n🔗 {getattr(message,'link','')}",
    ...
    async def handle_reply(self, client, message, notify: bool) -> None:
        ...
        if (group_id, reply_to_id) in self._pending_copy and text:
            ...
            self._log.info("[抢红包] 缓存口令 chat=%s reply=%s packet=%s 口令=%r",
    ...
    async def _handle_failure(self, group_id, reply_to_id, notify, client) -> None:
        ...
        self._log.info("[抢红包] OCR口令被判错 chat=%s 口令=%r", group_id, entry.keyword)
        # 失败后退回等待复制模式
    ...
    async def _handle_success(self, client, message, group_id, reply_to_id, notify) -> None:
        ...
        if ocr_entry is not None:
            ...
            self._log.info("[抢红包] OCR口令确认成功 chat=%s 口令=%r", group_id, ocr_entry.keyword)
        ...
        if is_trap_keyword(keyword, trap_enabled, custom, self._log):
            self._log.warning("[抢红包] 复制口令命中陷阱，拦截: %r", keyword)
            return
        ...
        try:
            ...
            self._log.error("[抢红包] 复制口令发送失败: %r", e)
            ok = False

        self._log.info("[抢红包] 复制模式发送 chat=%s packet=%s 口令=%r ok=%s",
    ...
    async def _ocr_timeout(self, group_id, sent_id) -> None:
        ...
        if removed is not None:
            self._log.info("[抢红包] OCR等待确认超时 chat=%s sent=%s 口令=%r",
    ...
    async def _download_image(self, client, message) -> bytes:
        ...
        try:
            ...
            self._log.debug("[抢红包] 图片下载失败: %r", e)
        return b""

    async def _safe_notify(self, text, level="info", account=None) -> None:
        try:
            await self._ctx.notify(text, level=level, category="抢红包", account=account)
